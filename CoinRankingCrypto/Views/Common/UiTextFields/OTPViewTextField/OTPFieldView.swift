...
@objc public class OTPFieldView: UIView {
    ...
    public var defaultBorderColor: UIColor = .label
    public var filledBorderColor: UIColor = .label
    public var errorBorderColor: UIColor?
    public var isOTPValid: Bool = true {
        didSet {
            showErrorView()
        }
    }
    ...
    override public func layoutSubviews() {
        ...
        if !hasInitialized, bounds.size != .zero {
            initializeUI()
            hasInitialized = true
        }
    }
    ...
    public func clearAllTextFields() {
        // Clear internal data storage for secure entry
        secureEntryData = Array(repeating: "", count: fieldsCount)

        // Loop through each OTP text field and clear the text
        for index in 0 ..< fieldsCount {
            if let otpField = viewWithTag(index + 1) as? OTPTextField {
                otpField.text = ""

                // Reset to default background and border color
                if displayType == .diamond || displayType == .underlinedBottom {
                    otpField.shapeLayer.fillColor = defaultBackgroundColor.cgColor
                    otpField.shapeLayer.strokeColor = defaultBorderColor.cgColor
                } else {
    ...
    fileprivate func getOTPField(forIndex index: Int) -> OTPTextField {
        ...
        otpField.otpCornerRadius = otpCornerRadius
    ...
    func showErrorView() {
        for index in stride(from: 0, to: fieldsCount, by: 1) {
            ...
            if !isOTPValid {
                otpField?.layer.borderColor = errorBorderColor?.cgColor
                otpField?.textColor = errorBorderColor
                otpField?.tintColor = errorBorderColor
            } else {
                otpField?.tintColor = filledBorderColor
            }
        }
    }
}

@objc public extension OTPFieldView {
    func updateFieldsAppearance() {
        UIView.performWithoutAnimation {
            for index in 0 ..< fieldsCount {
                if let otpField = self.viewWithTag(index + 1) as? OTPTextField {
                    let isEmpty = otpField.text?.isEmpty ?? true
                    let bgColor = isEmpty ? defaultBackgroundColor : filledBackgroundColor
                    let borderColor = isEmpty ? defaultBorderColor : filledBorderColor

                    if displayType == .diamond || displayType == .underlinedBottom {
                        otpField.shapeLayer.fillColor = bgColor.cgColor
                        otpField.shapeLayer.strokeColor = borderColor.cgColor
                    } else {
                        otpField.backgroundColor = bgColor
                        otpField.layer.borderColor = borderColor.cgColor
                    }
                }
            }
            self.layoutIfNeeded()
...
extension OTPFieldView: UITextFieldDelegate {
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if replacedText.count >= 1 {
            ...
            if let nextOTPField = viewWithTag(textField.tag + 1) {
            ...
            // User is attempting to delete (replacement string is empty)
            let currentText = textField.text ?? ""

            if currentText.isEmpty {
                // If the current field is empty, try moving focus to the previous field
                if let previousField = viewWithTag(textField.tag - 1) as? OTPTextField {
                    previousField.text = ""
                    previousField.becomeFirstResponder()
                    // Update secure entry data if needed
                    secureEntryData[previousField.tag - 1] = ""
                    calculateEnteredOTPSTring(isDeleted: true)
                }
                return false
            } else {
                // Delete character from the current field
    ...
    private func deleteText(in textField: UITextField) {
        // Clear secure entry data for this field
